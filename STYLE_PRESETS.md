...
`htmlppt` 的 12 套视觉预设。每套风格都尽量避免通用 AI 模板味，只保留抽象图形和可维护的 CSS 结构。
...
上面的展示字体多数是拉丁字体。**只要正文或标题里有明显中文，必须补一层 CJK fallback。**

推荐链路：

```css
--font-sans-cjk: "Noto Sans SC", "Source Han Sans SC", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "微软雅黑", sans-serif;
--font-serif-cjk: "Noto Serif SC", "Source Han Serif SC", "Songti SC", "STSong", "SimSun", serif;
```

推荐做法：

- 无衬线预设：展示字体后面接 `--font-sans-cjk`
- 衬线预设：展示字体后面接 `--font-serif-cjk`
- 如果标题是纯中文，不要只写 `Clash Display`、`Fraunces`、`Bodoni Moda` 这类拉丁字体

## 字体搭配速查

| Preset | Display Font | Body Font | Source | 中文建议 |
|--------|--------------|-----------|--------|-----------|
| Bold Signal | Archivo Black | Space Grotesk | Google | 接 `Noto Sans SC` |
| Electric Studio | Manrope | Manrope | Google | 接 `Noto Sans SC` |
| Creative Voltage | Syne | Space Mono | Google | 接 `Noto Sans SC` |
| Dark Botanical | Cormorant | IBM Plex Sans | Google | 标题接 `Noto Serif SC`，正文接 `Noto Sans SC` |
| Notebook Tabs | Bodoni Moda | DM Sans | Google | 标题接 `Noto Serif SC` |
| Pastel Geometry | Plus Jakarta Sans | Plus Jakarta Sans | Google | 接 `Noto Sans SC` |
| Split Pastel | Outfit | Outfit | Google | 接 `Noto Sans SC` |
| Vintage Editorial | Fraunces | Work Sans | Google | 标题接 `Noto Serif SC` |
| Neon Cyber | Clash Display | Satoshi | Fontshare | 必须补中文 fallback |
| Terminal Green | JetBrains Mono | JetBrains Mono | JetBrains | 仅适合少量中文或代码页 |
