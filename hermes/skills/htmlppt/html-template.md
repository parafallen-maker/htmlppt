...
生成 HTML 幻灯片时的参考骨架。所有演示默认遵循这个结构。
...
<html lang="en">
  <head>
    ...
    <!-- Fonts:
         1. 可以继续使用 Fontshare / Google Fonts 的拉丁展示字体
         2. 只要演示里包含中文，必须补 CJK 字体或 fallback 链
         3. 推荐同时加载 Noto Sans/Serif SC，减少中文回退失控 -->
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      rel="stylesheet"
      href="https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@400;500;700;900&family=Noto+Serif+SC:wght@400;600;700&display=swap"
    />
    ...
    <style>
      ...
      :root {
        ...
        --font-display:
          "Clash Display", "Noto Sans SC", "Source Han Sans SC",
          "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "微软雅黑",
          sans-serif;
        --font-body:
          "Satoshi", "Noto Sans SC", "Source Han Sans SC", "PingFang SC",
...
**如果用户不需要浏览器内改字，就不要生成任何编辑相关的 HTML / CSS / JS。**

**不要用 CSS `~` sibling selector 做 hover 显示按钮。** 纯 CSS 写法在 `pointer-events: none` 场景下会断掉 hover 链，用户鼠标一移过去按钮就消失。

**正确做法是：用 JS 控制 hover，并加 400ms 延迟隐藏。**
