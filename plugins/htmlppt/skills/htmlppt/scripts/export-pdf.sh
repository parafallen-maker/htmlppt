...
TEMP_DIR=$(mktemp -d)
TEMP_SCRIPT="$TEMP_DIR/export-slides.cjs"
...
// export-slides.cjs — Playwright script to export HTML slides to PDF
//
// How it works:
...
const { readFileSync, mkdirSync, unlinkSync } = require('fs');
...
// Find a free port, then run the export flow
(async () => {
...
for (let i = 0; i < slideCount; i++) {
  ...
  // Most htmlppt presentations use a currentSlide index and show/hide
  await page.evaluate((index) => {
    const slides = document.querySelectorAll('.slide');

    // Try multiple navigation strategies used by htmlppt:
...
# ─── Step 3: Resolve Playwright ────────────────────────────
# Prefer an existing Playwright installation. Only install a temporary copy
# when the machine does not already have a usable module.

info "Setting up Playwright (headless browser for screenshots)..."
info "Will reuse an existing Playwright if available..."
echo ""
...
resolve_current_playwright() {
    node - <<'NODE' 2>/dev/null || true
const path = require('path');
const pkg = require.resolve('playwright/package.json');
process.stdout.write(
  path.dirname(path.dirname(pkg)) + '\t' + path.join(path.dirname(pkg), 'cli.js')
);
NODE
}

resolve_global_playwright() {
    local global_root
    global_root=$(npm root -g 2>/dev/null || true)
    [ -z "$global_root" ] && return 1

    NODE_PATH="$global_root" node - <<'NODE' 2>/dev/null || true
const path = require('path');
const pkg = require.resolve('playwright/package.json');
process.stdout.write((process.env.NODE_PATH || '') + '\t' + path.join(path.dirname(pkg), 'cli.js'));
NODE
}

has_chromium_binary() {
    NODE_PATH="$1" node - <<'NODE' >/dev/null 2>&1
const fs = require('fs');
const { chromium } = require('playwright');
try {
  const executable = chromium.executablePath();
  if (executable && fs.existsSync(executable)) process.exit(0);
} catch (_) {}
process.exit(1);
NODE
}

CURRENT_PLAYWRIGHT=$(resolve_current_playwright)
if [[ -n "$CURRENT_PLAYWRIGHT" ]]; then
    IFS=$'\t' read -r PLAYWRIGHT_NODE_PATH PLAYWRIGHT_CLI_JS <<< "$CURRENT_PLAYWRIGHT"
    PLAYWRIGHT_SOURCE="current Node environment"
else
    GLOBAL_PLAYWRIGHT=$(resolve_global_playwright)
    if [[ -n "$GLOBAL_PLAYWRIGHT" ]]; then
        IFS=$'\t' read -r PLAYWRIGHT_NODE_PATH PLAYWRIGHT_CLI_JS <<< "$GLOBAL_PLAYWRIGHT"
        PLAYWRIGHT_SOURCE="global npm"
    fi
fi

if [[ -z "$PLAYWRIGHT_NODE_PATH" ]]; then
    info "Playwright not found, installing a temporary local copy..."
    cd "$TEMP_DIR"

    cat > "$TEMP_DIR/package.json" << 'PKG'
{ "name": "slide-export", "private": true }
PKG

    npm install playwright --no-save &>/dev/null || {
    ...
    PLAYWRIGHT_NODE_PATH="$TEMP_DIR/node_modules"
    PLAYWRIGHT_CLI_JS="$TEMP_DIR/node_modules/playwright/cli.js"
    PLAYWRIGHT_SOURCE="temporary local install"
fi

ok "Using Playwright from: $PLAYWRIGHT_SOURCE"

if has_chromium_binary "$PLAYWRIGHT_NODE_PATH"; then
    ok "Chromium browser already available"
else
    info "Chromium browser not found, installing..."
    NODE_PATH="$PLAYWRIGHT_NODE_PATH" node "$PLAYWRIGHT_CLI_JS" install chromium 2>/dev/null || {
...
NODE_PATH="$PLAYWRIGHT_NODE_PATH" node "$TEMP_SCRIPT" "$SERVE_DIR" "$HTML_FILENAME" "$OUTPUT_PDF" "$SCREENSHOT_DIR" "$VIEWPORT_W" "$VIEWPORT_H" || {
